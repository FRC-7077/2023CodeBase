...
//   private double kD = 0.0;
//   private double kIz = 1;
//   private double kFF = 0.000556; // Aggressive Acceleration...for more moderate acceleration use 0.000256
//   private double kMaxOutput = 1;
//   private double kMinOutput = -1;
...
//   }  
  ...
  //   double appliedAxis = Math.abs(Math.sin(navX2.getYaw())) >= Math.abs(Math.cos(navX2.getYaw())) ? leftRightAxis : forwardBackwardAxis;
    
  //   double left = clamp((appliedAxis + twistAxis + steering_adjust - distance_adjust), -1, 1) * maxRPM;
  //   double right = clamp((appliedAxis - twistAxis - steering_adjust - distance_adjust), -1, 1) * maxRPM;

  //   // Checking to see if you need Backup
  //   if(needBackup)
  //   {
  //     left = joyBackup.getRawAxis(1) * maxRPM;
  //     right = joyBackup.getRawAxis(3) * maxRPM;
  //   }
...
//   public void balance()
//   {
//     double pitch_error = -1 * (navX2.getPitch() * Math.PI / 180);

//     // Set Adjustment for Pitch Correction
//     distance_adjust = Math.abs(navX2.getPitch()) > 10 ? pitch_error : 0;
//   }
